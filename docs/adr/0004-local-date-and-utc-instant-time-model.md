...
Church Task models church-calendar concepts local-date-first while storing UTC instants for clock comparisons and scheduled jobs. A Cycle is identified by the Church-local Monday `start_date` it represents, while UTC boundary instants may be stored for scheduling and comparisons. This keeps user-facing week identity stable even if a Church changes time zone later.
